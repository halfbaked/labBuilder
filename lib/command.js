(function() {
  var LabBuilder, NOMNOM_CONFIG, fs, helpers, nomnom, printLine;
  ...
  exports.run = function() {
    ...
    console.log(options);
    if (options.scripts) {
      return fs.readFile(options.scripts, encoding = 'utf8', function(err, data) {
        var scriptsJson;
        if (err) {
          throw err;
        }
        scriptsJson = JSON.parse(helpers.trimString(data));
        return LabBuilder.build(options.labjs, scriptsJson, options.output);
      });
    }
  };
}).call(this);
